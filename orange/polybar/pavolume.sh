...
VOLUME_ICONS=( "\UF027 " "\UF028 " "\UF028 " )  # Volume icons array, from lower volume to higher
MUTED_ICON="\UF6A9 "  # Muted volume icon
...
function output() {
    ...
    if [ "$isMuted" = "yes" ]; then
        echo -e "${MUTED_COLOR}${MUTED_ICON}${END_COLOR}"
    else
        echo -e "${volIcon}${curVol}%"
    fi
}
